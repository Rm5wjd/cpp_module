...
bool Bureaucrat::signForm(const std::string& formName, int signGrade)
{
	if (grade <= signGrade) {
		...
		std::cout << name << " couldn't sign " << formName << " because " << "grade too low, Form's grade " << signGrade << ", bureaucrat grade " << grade << std::endl;
		return false;
	}
}
